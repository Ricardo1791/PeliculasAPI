...
namespace PeliculasAPI.Tests.PruebasDeIntegracion
{
    ...
    public class GenerosControllerTest: BasePruebas
    {
        ...
        public async Task ObtenerTodosLosGeneros()
        {
            ...
            contexto.Generos.Add(new Entities.Genero() { Nombre = "Genero 1" });
            contexto.Generos.Add(new Entities.Genero() { Nombre = "Genero 2" });
        ...
        public async Task BorrarGeneroRetorna401()
        {
            ...
            var cliente = factory.CreateClient();
            var respuesta = await cliente.DeleteAsync($"{url}/1");
            Assert.AreEqual("Unauthorize", respuesta.ReasonPhrase);
        }
    }
}
